...
5. 効果：「どの」施策の効果が大きいのか（根本原因は何か、中核問題は何か、優先順位）
...
2. 問題：「何に」困っているのか
    ...
    - 「何ができれば解決できそうですか？」
3. 原因：「なぜ」それが起きるのか
    - 「うまくいかなくなるのは、何をしたときですか？」
    - 「うまくいかなくなる場面やタイミングなどはありますか？」
    - 「いつもうまくいきませんか？それとも、うまくいくこともありますか？」
    - 「ジャマをしている前提条件や制約条件はありますか？」
    - 「根拠になるような情報はありますか？調べられそうですか？」
    - 「誰がやっても同じ結果になりそうですか？」
4. 施策：「どんな」解決策があるのか（どんなことができそうか）
    - 「たとえば、どんなことをすれば解決できそうですか？」
    - 「頭の中でなんとなく出てきていることはありますか？」
    - 「みんなが思いついているかもしれないような、当たり前な方法はありますか？」
    - 「できないかもしれないけど、思いついたものはありますか？」
    - 「〇〇さん、△△について、またはそれ以外について感じたことがあれば教えてください」
    - 「今まで出てきた内容で、理解しきれていないことはありますか？」
    - 「今まで出てきた内容で、あえて付け足せそうなアイデアや意見はありますか？」
    - 「具体的にまだイメージできていない施策はありますか？」
5. 効果：「どの」施策の効果が大きいのか（根本原因は何か、中核問題は何か、優先順位）
    - 「パッとみて、この中でいちばん費用対効果が高そうなものはありますか？」
    - 「この中で費用対効果が高そうなものをいくつかピックアップしたいと思います。確実○票ずつ持って、施策につき1票ずつ投票してください」
    - 「依存関係や順番を決めたほうがよさそうな施策はありますか？」
...
#### ミスの3H
- 初めて（品質不足、スケジュールの超過）：事前の準備、余裕のあるスケジュール、レビュー体制
...
4. 劇場のイドラ：伝統や権威を重んじて、無批判に正しいと受けれる偏見
...
3. 説明：
3.1【経緯】：「なぜ」課題を追加したのか
3.2【発生状況】「どんな」状況で問題が発生するのか（省略可）
3.3【完了条件】「何を」解決すれば完了できるのか
3.4【情報】完了するために「必要な情報」はあるか
3.5【懸念点】「障害」になりそうなことはあるか（省略可）
4. 期日：「いつまで」に完了するか
...
1. 要求度（高） × 裁量度（低）：（高緊張）ストレスが溜まりやすい働き方→まわりのサポートで緩和する
2. 要求度（高） × 裁量度（高）：（能動的）大変だけど、達成感がありストレスを溜めにくい働き方
3. 要求度（低） × 裁量度（低）：（受動的）楽だけど、やる気が削がれる働き方
4. 要求度（低） × 裁量度（高）：（低緊張）ストレスが溜まりにくい働き方
...
単純や煩雑であればウォーターフォール、複雑であればアジャイル。
...
1. 自分の意見を誰よりも早く出す人
2. 自ら変化を起こす人
3. 最も成果に繋がることにリソースを集中する人
4. 目的と目標（定量、定性）、行動を促す明確な戦略を指し示す人
5. 不確実性が高くても決断する人
6. 困難でもやり遂げる人

大変な状況でも「この人に付いていきたい」と思わせる人がリーダーシップのある人なのだと思う。
